...
class DataEvaluator:
    ...
    def generate_evaluation_plots(self):
        ...
        try:
            ...
            plt.subplot(1, 2, 1)
            sns.barplot(data=data_mean, x='Feature', y='Mean', hue='Type')
            plt.xticks(rotation=45)
            plt.title('Mean Comparison')

            plt.subplot(1, 2, 2)
            sns.barplot(data=data_std, x='Feature', y='Std', hue='Type')
            plt.xticks(rotation=45)
            ...
            numeric_cols = self.real_processed.select_dtypes(include=[np.number]).columns
            n_cols = min(2, len(numeric_cols))
            n_rows = (len(numeric_cols) + n_cols - 1) // n_cols

            fig_cumsums = plt.figure(figsize=(15, 5*n_rows))

            for i, col in enumerate(numeric_cols):
                plt.subplot(n_rows, n_cols, i + 1)

                # Create CDF data
                real_sorted = np.sort(self.real_processed[col])
                synth_sorted = np.sort(self.synthetic_processed[col])
                real_cdf = np.arange(1, len(real_sorted) + 1) / len(real_sorted)
                synth_cdf = np.arange(1, len(synth_sorted) + 1) / len(synth_sorted)

                # Plot CDFs using seaborn
                sns.lineplot(x=real_sorted, y=real_cdf, label='Real')
                sns.lineplot(x=synth_sorted, y=synth_cdf, label='Synthetic')
            ...
            plt.tight_layout()
            figures.extend([fig_mean_std, fig_cumsums])
