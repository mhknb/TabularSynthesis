...
class DataTransformer:
    ...
    def transform_categorical(self, data: pd.Series, method: str = 'label') -> pd.Series:
        ...
        if method == 'label':
            ...
            # For one-hot encoding, we return just the first dummy column
            # and store the full mapping for later reconstruction
    ...
    def inverse_transform_categorical(self, data: pd.Series) -> pd.Series:
        ...
        if encoding_info['method'] == 'label':
            ...
            # For one-hot encoding, we map back to the original categories
            return pd.Series(encoding_info['original_categories'][data.astype(int)], name=data.name)
